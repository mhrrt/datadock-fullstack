...
export async function exportRecordsToExcel(
  ...
  exportData.forEach((row) => {
    worksheet.addRow({
      ...
      name: row.name,
      codeName: row.codeName.toUpperCase(),
      codeNum: row.codeNum,
      state: row.state?.name.toUpperCase() ?? "",
      city: row.city?.name.toUpperCase() ?? "",
      ...
      referenceName: row.referenceName.toUpperCase(),
      byWhom: row.byWhom,
      remark: row.remark.toUpperCase(),
      // for pending amount etc
      pendingAmount: row.pendingAmount,
      receivedAmount: row.receivedAmount,
      outstandingAmount: row.outstandingAmount,
      recoveryRemark: row.recoveryRemark.toUpperCase(),
      status: row.status,
      createdByName: row.createdBy?.fullName ?? "",
    });
  });
